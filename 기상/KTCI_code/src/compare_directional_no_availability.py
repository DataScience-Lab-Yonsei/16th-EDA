"""방향성 Spearman + 가용률 미반영 대안 KTCI 비교.

새 증거점수:
    E = sqrt(max(0, -Spearman) * decile_amplitude)

원 데이터·관광잔차·점수변환·결측행 처리 방식은 기존 파이프라인과 동일하다.
가용률은 가중치 산식에서만 제외하며, 개별 날짜의 결측값을 0으로 채우지는 않는다.
"""
...
new_weights["new_weight_directional_no_availability"] = new_weights.groupby("season")[
    "new_evidence_no_availability"
].transform(lambda x: x / x.sum())
new_weights["weight_change_pp"] = (
    new_weights.new_weight_directional_no_availability - new_weights.current_weight
) * 100
...
for season in cfg["analysis"]["seasons"]:
    ...
    df.loc[idx, "KTCI_new_directional"] = weighted_available(scores.loc[idx], new_w, 2)
...
common = df.dropna(subset=["KTCI_current", "KTCI_new_directional", "KTCI_2014_adapted"]).copy()
performance_rows = []
for season in ["all", "spring", "summer", "autumn", "winter"]:
    d = common if season == "all" else common[common.season.eq(season)]
    for col in ["KTCI_current", "KTCI_new_directional", "KTCI_2014_adapted"]:
...
new_weights.to_csv(TABLES / "alternative_directional_no_availability_weights.csv",
                   index=False, encoding="utf-8-sig")
performance_df.to_csv(TABLES / "alternative_directional_no_availability_performance.csv",
                      index=False, encoding="utf-8-sig")

summary = {
    "formula": "sqrt(max(0,-spearman) * decile_amplitude); availability excluded",
    "common_rows": len(common),
    "outputs": [
        "alternative_directional_no_availability_weights.csv",
        "alternative_directional_no_availability_performance.csv",
    ],
}
(OUT / "logs" / "alternative_comparison_metadata.json").write_text(
